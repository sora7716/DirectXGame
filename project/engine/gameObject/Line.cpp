...
//初期化
void Line::Initialize(DirectXBase* directXBase) {
...
// DirectXの基盤のゲッター
DirectXBase* Line::GetDirectXBase() const {
	return directXBase_;
}

// ルートシグネイチャの設定
void Line::InitializeRootSigneture() {
	//RootSignature作成
...
void Line::CreateGraphicsPipeline() {
	...
	//Shaderをコンパイルする
	//VertexShader
	Microsoft::WRL::ComPtr<IDxcBlob> vertexShaderBlob = directXBase_->CompilerShader(L"engine/resources/shaders/Line.VS.hlsl", L"vs_6_0");
	assert(vertexShaderBlob != nullptr);

	//PixelShader
	Microsoft::WRL::ComPtr<IDxcBlob> pixelShaderBlob = directXBase_->CompilerShader(L"engine/resources/shaders/Line.PS.hlsl", L"ps_6_0");
